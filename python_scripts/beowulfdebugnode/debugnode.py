...
from beowulfapi.beowulfnoderpc import BeowulfNodeRPC

class DebugNode( object ):
   """ Wraps the beowulfd debug node plugin for easier automated testing of the Steem Network"""

   def __init__( self, beowulfd, data_dir, args='', plugins=[], apis=[], beowulfd_out=None, beowulfd_err=None ):
      """ Creates a beowulfd debug node.
      ...
      For all other requests, the python-beowulf library should be used.

      args:
         beowulfd -- The string path to the location of the beowulfd binary
         data_dir -- The string path to an existing beowulfd data directory which will be used to pull blocks from.
         args -- Other string args to pass to beowulfd.
         ...
         beowulfd_stdout -- A stream for beowulfd's stdout. Default is to pipe to /dev/null
         beowulfd_stderr -- A stream for beowulfd's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._beowulfd_bin = None
      self._beowulfd_lock = None
      self._beowulfd_process = None
      self._temp_data_dir = None

      self._beowulfd_bin = Path( beowulfd )
      if( not self._beowulfd_bin.exists() ):
         raise ValueError( 'beowulfd does not exist' )
      if( not self._beowulfd_bin.is_file() ):
         raise ValueError( 'beowulfd is not a file' )

      self._data_dir = Path( data_dir )
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed beowulf data directory' )
      ...
      if( beowulfd_out != None ):
         self.beowulfd_out = beowulfd_out
      else:
         self.beowulfd_out = self._FNULL

      if( beowulfd_err != None ):
         self.beowulfd_err = beowulfd_err
      else:
         self.beowulfd_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._beowulfd_lock = Lock()


   def __enter__( self ):
      self._beowulfd_lock.acquire()
      ...
      beowulfd = [ str( self._beowulfd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      beowulfd.extend( self._args )

      self._beowulfd_process = Popen( beowulfd, stdout=self.beowulfd_out, stderr=self.beowulfd_err )
      self._beowulfd_process.poll()
      sleep( 5 )
      if( not self._beowulfd_process.returncode ):
         self._rpc = BeowulfNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "beowulfd did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._beowulfd_process != None ):
         self._beowulfd_process.poll()

         if( not self._beowulfd_process.returncode ):
            self._beowulfd_process.send_signal( SIGINT )

            sleep( 7 )
            self._beowulfd_process.poll()

            if( not self._beowulfd_process.returncode ):
               self._beowulfd_process.send_signal( SIGTERM )

               sleep( 5 )
               self._beowulfd_process.poll()

               if( self._beowulfd_process.returncode ):
                  loggin.error( 'beowulfd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._beowulfd_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._beowulfd_lock.release()
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key beowulf debug`. Do not use this key on the live chain for any reason.
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is BEOWULF_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both beowulfd and the data directory,
      ...
      parser = ArgumentParser( description='Run a Debug Node on an existing chain. This simply replays all blocks ' + \
                                 ...
                                 'the CLI wallet' )
      parser.add_argument( '--beowulfd', '-s', type=str, required=True, help='The location of a beowulfd binary to run the debug node' )
      ...
      beowulfd = Path( args.beowulfd )
      if( not beowulfd.exists() ):
         print( 'Error: beowulfd does not exist.' )
         return

      beowulfd = beowulfd.resolve()
      if( not beowulfd.is_file() ):
         print( 'Error: beowulfd is not a file.' )
         return

      data_dir = Path( args.data_dir )
      if( not data_dir.exists() ):
         print( 'Error: data_dir does not exist or is not a properly constructed beowulfd data directory' )
      ...
      debug_node = DebugNode( str( beowulfd ), str( data_dir ), beowulfd_err=sys.stderr )
