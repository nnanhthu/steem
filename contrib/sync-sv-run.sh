...
VERSION=`cat /etc/beowulfdversion`

# if the writer node dies by itself, kill runsv causing the container to exit
BEOWULFD_PID=`pgrep -f p2p-endpoint`
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! beowulfdsync has quit unexpectedly, checking for coredump and then starting a new instance..
  sleep 30
  SAVED_PID=`cat /tmp/beowulfdpid`
  if [[ -e /tmp/core.$SAVED_PID ]]; then
    gdb --batch --quiet -ex "thread apply all bt full" -ex "quit" /usr/local/beowulfd-full/bin/beowulfd /tmp/core.$SAVED_PID >> /tmp/stacktrace
    STACKTRACE=`cat /tmp/stacktrace`
    echo NOTIFYALERT! beowulfdsync stacktrace from coredump:
  ...
  RUN_SV_PID=`pgrep -f /etc/service/beowulfd`
  kill -9 $RUN_SV_PID
fi
...
if [[ ! -z "$BLOCKCHAIN_TIME" ]]; then
  ...
  if [[ ${BLOCK_AGE} -le 10 ]]; then
    BEOWULFD_PID=`pgrep -f p2p-endpoint`
    kill -SIGINT $BEOWULFD_PID
    echo beowulfdsync: waiting for beowulfd to exit cleanly

    # wait 60 seconds for beowulfd to exit, to be safe.
    let WAIT_TIME=0
    while ( kill -0 $BEOWULFD_PID ) && [[ WAIT_TIME -le 60 ]]; do
       sleep 1
       let WAIT_TIME++
    done

    echo beowulfdsync: starting a new blockchainstate upload operation
    cd ${COMPRESSPATH:-$HOME}
    echo beowulfdsync: compressing blockchainstate...
    if [[ "$USE_RAMDISK" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! beowulfdsync was unable to compress shared memory file, check the logs.
      exit 1
    fi
    ...
    echo beowulfdsync: uploading $FILE_NAME to $S3_BUCKET
    aws s3 cp blockchain.tar.lz4 s3://$S3_BUCKET/$FILE_NAME
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! beowulfdsync was unable to upload $FILE_NAME to s3://$S3_BUCKET
      exit 1
    fi
    echo beowulfdsync: replacing current version of blockchain state with $FILE_NAME
    if [[ "$IS_BROADCAST_NODE" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! beowulfdsync was unable to overwrite the current blockchainstate with $FILE_NAME
      exit 1
    fi
    ...
    echo beowulfdsync: stopping the container after a sync operation
    if [[ -e /tmp/isnewsync ]]; then
      echo notifybeowulfdsync: beowulfdsync: successfully generated and uploaded new blockchain-$VERSION-latest.tar.lz4 to s3://$S3_BUCKET
    fi
    RUN_SV_PID=`pgrep -f /etc/service/beowulfd`
    kill -9 $RUN_SV_PID
  fi
