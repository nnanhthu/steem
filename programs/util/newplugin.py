...
target_link_libraries( {plugin_provider}_{plugin_name} beowulfit_app beowulfit_chain beowulfit_protocol )
...
{{
   public:
      {plugin_name}_api( const beowulf::app::api_context& ctx );
...
class {plugin_name}_plugin : public beowulf::app::plugin
{{
   public:
      {plugin_name}_plugin( beowulf::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <beowulf/app/api_context.hpp>
#include <beowulf/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( beowulf::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      beowulf::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( beowulf::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const beowulf::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( beowulf::app::application& app );
...
{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( beowulf::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( beowulf::app::application* app )
   : plugin(app)
{{
...
BEOWULF_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider (beowulf for plugins developed by ULI)")
