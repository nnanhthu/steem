...
#include <beowulf/plugins/rc/rc_export_objects.hpp>
#include <beowulf/plugins/rc/rc_plugin.hpp>
#include <beowulf/plugins/rc/rc_objects.hpp>
...
#include <beowulf/jsonball/jsonball.hpp>

#define BEOWULF_RC_REGEN_TIME   (60*60*24*5)
// 2020.748973 VESTS == 1.000 BEOWULF when HF20 occurred on mainnet
// TODO: What should this value be for testnet?
#define BEOWULF_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT      2020748973

#ifndef IS_TEST_NET
#define BEOWULF_HF20_BLOCK_NUM                              26256743
#endif

// 1.66% is ~2 hours of regen.
// 2 / ( 24 * 5 ) = 0.01666...
#define BEOWULF_RC_MAX_NEGATIVE_PERCENT 166

namespace beowulf { namespace plugins { namespace rc {

using beowulf::plugins::block_data_export::block_data_export_plugin;

namespace detail {

using chain::plugin_exception;
using beowulf::chain::util::manabar_params;
...
void create_rc_account( database& db, uint32_t now, const account_object& account, asset max_rc_creation_adjustment )
{
   ...
   {
      //Convert from sbd to beowulf
      max_rc_creation_adjustment = db.to_beowulf(max_rc_creation_adjustment);
   }

   if( max_rc_creation_adjustment.symbol == BEOWULF_SYMBOL )
   {
   ...
   rc_plugin_skip_flags skip )
{
   if( account_name == account_name_type() )
   {
      if( db.is_producing() )
      {
         BEOWULF_ASSERT( false, plugin_exception,
   ...
   mbparams.regen_time = BEOWULF_RC_REGEN_TIME;
   ...
   db.modify( rc_account, [&]( rc_account_object& rca )
   {
      ...
      if( (!skip.skip_reject_not_enough_rc) && db.has_hardfork( BEOWULF_HARDFORK_0_20 ) )
      {
         if( db.is_producing() )
         {
            BEOWULF_ASSERT( has_mana, plugin_exception,
               "Account: ${account} has ${rc_current} RC, needs ${rc_needed} RC. Please wait to transact, or power up BEOWULF.",
      ...
      int64_t min_mana = -1 * ( BEOWULF_RC_MAX_NEGATIVE_PERCENT * mbparams.max_mana ) / BEOWULF_100_PERCENT;
...
void rc_plugin_impl::on_post_apply_transaction( const transaction_notification& note )
{
   ...
   int64_t rc_regen = (gpo.total_vesting_shares.amount.value / (BEOWULF_RC_REGEN_TIME / BEOWULF_BLOCK_INTERVAL));
   ...
   if( rc_regen > 0 )
   {
      for( size_t i=0; i<BEOWULF_NUM_RESOURCE_TYPES; i++ )
      {
   ...
   std::shared_ptr< exp_rc_data > export_data =
      beowulf::plugins::block_data_export::find_export_data< exp_rc_data >( BEOWULF_RC_PLUGIN_NAME );
...
//               a /= k*double(pool);
//               dlog( "a=${a}   aR=${aR}", ("a", a)("aR", a*gpo.total_vesting_shares.amount.value/BEOWULF_RC_REGEN_TIME) );
//            }
//         }
//         if( debug_print )
//         {
...
void rc_plugin_impl::on_first_block()
{
   ...
   std::string resource_params_json = beowulf::jsonball::get_resource_parameters();
   ...
   for( auto it=idx.begin(); it!=idx.end(); ++it )
   {
      create_rc_account( _db, now.sec_since_epoch(), *it, asset( BEOWULF_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT, VESTS_SYMBOL ) );
   }
...
struct pre_apply_operation_visitor
{
   ...
   void regenerate( const account_object& account, const rc_account_object& rc_account )const
   {
      ...
      // TODO:  Issue number
      //
      static_assert( BEOWULF_RC_REGEN_TIME <= BEOWULF_VOTING_MANA_REGENERATION_SECONDS, "RC regen time must be smaller than vote regen time" );
...
//#endif
   ...
   {
      if( op.hardfork_id == BEOWULF_HARDFORK_0_1 )
      {
         const auto& idx = _db.get_index< account_index >().indices().get< by_id >();
         for( auto it=idx.begin(); it!=idx.end(); ++it )
         {
...
//   }
   ...
   {
      regenerate( BEOWULF_NULL_ACCOUNT );
   }
...
//      create_rc_account< true >( _db, _current_time, op.worker_account, asset( 0, BEOWULF_SYMBOL ) );
...
//      create_rc_account< true >( _db, _current_time, worker_name, asset( 0, BEOWULF_SYMBOL ) );
...
//#endif
   ...
   void operator()( const hardfork_operation& op )const
   {
      ...
      if( op.hardfork_id == BEOWULF_HARDFORK_0_20 )
      {
...
//   }
   ...
   void operator()( const clear_null_account_balance_operation& op )const
   {
      _mod_accounts.emplace_back( BEOWULF_NULL_ACCOUNT );
   }
...
void rc_plugin_impl::on_pre_apply_operation( const operation_notification& note )
{
   ...
   if( _db.has_hardfork( BEOWULF_HARDFORK_0_20 ) )
...
void rc_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      if( export_plugin != nullptr )
      {
         ...
         export_plugin->register_export_data_factory( BEOWULF_RC_PLUGIN_NAME,
            []() -> std::shared_ptr< exportable_block_data > { return std::make_shared< exp_rc_data >(); } );
      }

      chain::database& db = appbase::app().get_plugin< beowulf::plugins::chain::chain_plugin >().db();
      ...
      {
         my->_enable_at_block = BEOWULF_HF20_BLOCK_NUM;
      }
#endif
