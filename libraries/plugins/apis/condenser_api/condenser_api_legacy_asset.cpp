...
uint32_t string_to_asset_num( const char* p, uint8_t decimals )
{
   ...
   switch( *p )
   {
      ...
      case 'S': case 'T': case 'U': case 'V': case 'W': case 'X': case 'Y': case 'Z':
      {
         ...
         switch( name_u64 )
         {
            case BEOWULF_SYMBOL_U64:
               FC_ASSERT( decimals == 5, "Incorrect decimal places" );
               asset_num = BEOWULF_ASSET_NUM_BEOWULF;
               break;
            case SBD_SYMBOL_U64:
               FC_ASSERT( decimals == 5, "Incorrect decimal places" );
               asset_num = BEOWULF_ASSET_NUM_SBD;
               break;
            case VESTS_SYMBOL_U64:
               FC_ASSERT( decimals == 6, "Incorrect decimal places" );
               asset_num = BEOWULF_ASSET_NUM_VESTS;
               break;
            default:
