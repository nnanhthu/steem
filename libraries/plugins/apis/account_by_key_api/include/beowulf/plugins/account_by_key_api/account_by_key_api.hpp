...
{
   std::vector< beowulf::protocol::public_key_type > keys;
};

struct get_key_references_return
{
   std::vector< std::vector< beowulf::protocol::account_name_type > > accounts;
};
...
FC_REFLECT( beowulf::plugins::account_by_key::get_key_references_args,
   (keys) )

FC_REFLECT( beowulf::plugins::account_by_key::get_key_references_return,
   (accounts) )
