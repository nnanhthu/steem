...
//            FC_ASSERT(permlink.size() > STEEM_MIN_PERMLINK_LENGTH && permlink.size() < STEEM_MAX_PERMLINK_LENGTH,
...
//            dest.sbd_interest_rate = src.sbd_interest_rate;
        ...
        void witness_update_evaluator::do_apply(const witness_update_operation &o) {
            ...
            if (_db.has_hardfork(STEEM_HARDFORK_0_14__410)) {
                ...
                if (_db.has_hardfork(STEEM_HARDFORK_0_20__2651) || _db.is_producing()) {
                    ...
                    FC_ASSERT(o.props.account_creation_fee.amount <= STEEM_MAX_ACCOUNT_CREATION_FEE,
        ...
        void witness_set_properties_evaluator::do_apply(const witness_set_properties_operation &o) {
            FC_ASSERT(_db.has_hardfork(STEEM_HARDFORK_0_20__1620),
            ...
            if (flags.account_creation_changed) {
                ...
                if (_db.has_hardfork(STEEM_HARDFORK_0_20__2651) || _db.is_producing()) {
                    FC_TODO("Move to validate() after HF20");
                    FC_ASSERT(props.account_creation_fee.amount <= STEEM_MAX_ACCOUNT_CREATION_FEE,
                              "account_creation_fee greater than maximum account creation fee");
                }
            }
...
//            FC_ASSERT(!_db.has_hardfork(STEEM_HARDFORK_0_20__1760),
...
//                    FC_ASSERT(_db.head_block_time() - account_auth.last_owner_update > STEEM_OWNER_UPDATE_LIMIT,
...
//                      "Account does not have sufficient Steem Power for withdraw.");
            ...
            if (!_db.has_hardfork(STEEM_HARDFORK_0_20__1860) && !account.mined &&
                _db.has_hardfork(STEEM_HARDFORK_0_1)) {
                const auto &props = _db.get_dynamic_global_properties();
                const witness_schedule_object &wso = _db.get_witness_schedule_object();
                ...
                FC_ASSERT(account.vesting_shares > min_vests ||
                          (_db.has_hardfork(STEEM_HARDFORK_0_16__562) && o.vesting_shares.amount == 0),
            ...
            if (o.vesting_shares.amount == 0) {
                ...
                _db.modify(account, [&](account_object &a) {
                    ...
                    a.next_vesting_withdrawal =
                            _db.head_block_time() + fc::seconds(STEEM_VESTING_WITHDRAW_INTERVAL_SECONDS);
        ...
        void set_withdraw_vesting_route_evaluator::do_apply(const set_withdraw_vesting_route_operation &o) {
            try {
                ...
                if (itr == wd_idx.end()) {
                    ...
                    FC_ASSERT(from_account.withdraw_routes < STEEM_MAX_WITHDRAW_ROUTES,
                ...
                FC_ASSERT(total_percent <= STEEM_100_PERCENT,
...
//                FC_ASSERT(voter.can_vote, "Account has declined its voting rights.");
            ...
            if (itr == by_account_witness_idx.end()) {
                ...
                if (_db.has_hardfork(STEEM_HARDFORK_0_2)) {
                    FC_ASSERT(voter.witnesses_voted_for < STEEM_MAX_ACCOUNT_WITNESS_VOTES,
...
//            }
//
//            FC_ASSERT(o.props.maximum_block_size >= STEEM_MIN_BLOCK_SIZE_LIMIT * 2,
...
//            FC_ASSERT(_db.has_hardfork(STEEM_HARDFORK_0_20__1771),
...
//
//                    FC_ASSERT(current_witness.available_witness_account_subsidies >= STEEM_ACCOUNT_SUBSIDY_PRECISION,
...
//                }
//
//                FC_ASSERT(gpo.available_account_subsidies >= STEEM_ACCOUNT_SUBSIDY_PRECISION,
...
//                FC_ASSERT(_db.head_block_time() - account.last_account_recovery > STEEM_OWNER_UPDATE_LIMIT,
//                          "Owner authority can only be updated once an hour.");
//
//            const auto &recovery_request_idx = _db.get_index<account_recovery_request_index>().indices().get<by_account>();
//            auto request = recovery_request_idx.find(o.account_to_recover);
...
//
//            FC_ASSERT(from.savings_withdraw_requests < STEEM_SAVINGS_WITHDRAW_REQUEST_LIMIT,
