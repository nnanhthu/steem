 ...
 */
#pragma once
#include <beowulf/chain/block_log.hpp>
#include <beowulf/chain/fork_database.hpp>
#include <beowulf/chain/global_property_object.hpp>
#include <beowulf/chain/hardfork_property_object.hpp>
#include <beowulf/chain/node_property_object.hpp>
#include <beowulf/chain/notifications.hpp>

#include <beowulf/chain/util/advanced_benchmark_dumper.hpp>
#include <beowulf/chain/util/signal.hpp>

#include <beowulf/protocol/protocol.hpp>
...
namespace beowulf { namespace chain {
   ...
   using beowulf::protocol::operation;
   ...
   class database : public chainbase::database
   {
      public:
         ...
         struct open_args
         {
            ...
            uint64_t initial_supply = BEOWULF_INIT_SUPPLY;
            uint64_t initial_sbd_supply = SBD_INIT_SUPPLY;
...
//         uint32_t                   get_pow_summary_target()const;
         ...
         chain_id_type beowulf_chain_id = BEOWULF_CHAIN_ID;
...
//         void max_bandwidth_per_share()const;
      ...
      public:
          ...
          *
          * Passing slot_num == 0 returns BEOWULF_NULL_WITNESS
          */
         ...
         /** @return the sbd created and deposited to_account, may return BEOWULF if there is no median feed */
         std::pair< asset, asset > create_sbd( const account_object& to_account, asset beowulf, bool to_reward_balance=false );
         asset create_vesting( const account_object& to_account, asset beowulf, bool to_reward_balance=false );
...
//         share_type pay_reward_funds( share_type reward );
          ...
          * BEOWULF.  Return 0 SBD if there isn't a current_median_history
          */
         asset to_sbd( const asset& beowulf )const;
         asset to_beowulf( const asset& sbd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = BEOWULF_INIT_SUPPLY, uint64_t initial_sbd_supply = SBD_INIT_SUPPLY );
...
//#endif
         ...
         fc::time_point_sec            _hardfork_times[ BEOWULF_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ BEOWULF_NUM_HARDFORKS + 1 ];
