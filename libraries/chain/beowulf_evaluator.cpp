...
//            FC_ASSERT(permlink.size() > BEOWULF_MIN_PERMLINK_LENGTH && permlink.size() < BEOWULF_MAX_PERMLINK_LENGTH,
...
//            dest.sbd_interest_rate = src.sbd_interest_rate;
        ...
        void witness_update_evaluator::do_apply(const witness_update_operation &o) {
            ...
            if (_db.has_hardfork(BEOWULF_HARDFORK_0_14__410)) {
                FC_ASSERT(o.props.account_creation_fee.symbol.is_canon());
                if (_db.has_hardfork(BEOWULF_HARDFORK_0_20__2651) || _db.is_producing()) {
                    FC_TODO("Move to validate() after HF20");
                    FC_ASSERT(o.props.account_creation_fee.amount <= BEOWULF_MAX_ACCOUNT_CREATION_FEE,
            ...
            if (_db.is_producing() || _db.has_hardfork(BEOWULF_HARDFORK_0_20__2642)) {
                FC_ASSERT(o.props.maximum_block_size <= BEOWULF_SOFT_MAX_BLOCK_SIZE,
                          "Max block size cannot be more than 2MiB");
            }
            //Add constraint: Node must have at least BEOWULF_MIN_WITNESS_FUND vested coins to become a witness
            //Change BEOWULF_MIN_WITNESS_FUND from 3000000 BEOWULF to VESTS
            ...
            asset liquid = asset(BEOWULF_MIN_WITNESS_FUND, BEOWULF_SYMBOL);
        ...
        void witness_set_properties_evaluator::do_apply(const witness_set_properties_operation &o) {
            FC_ASSERT(_db.has_hardfork(BEOWULF_HARDFORK_0_20__1620),
                      "witness_set_properties_evaluator not enabled until HF 20");
            ...
            if (flags.account_creation_changed) {
                ...
                if (_db.has_hardfork(BEOWULF_HARDFORK_0_20__2651) || _db.is_producing()) {
                    FC_TODO("Move to validate() after HF20");
                    FC_ASSERT(props.account_creation_fee.amount <= BEOWULF_MAX_ACCOUNT_CREATION_FEE,
                              "account_creation_fee greater than maximum account creation fee");
                }
            }
...
//                }
        ...
        void initialize_account_object(account_object &acc, const account_name_type &name,
                                       const dynamic_global_property_object &props, bool mined,
                                       uint32_t hardfork) {
            acc.name = name;
//            acc.memo_key = key;
            ...
            if (hardfork < BEOWULF_HARDFORK_0_20__2539) {
                acc.voting_manabar.current_mana = BEOWULF_100_PERCENT;
            }

//            if (hardfork >= BEOWULF_HARDFORK_0_11) {
//                FC_TODO("If after HF 20, there are no temp account creations, the HF check can be removed.")
//                if ((hardfork < BEOWULF_HARDFORK_0_20__1782) || (recovery_account != BEOWULF_TEMP_ACCOUNT)) {
//                    acc.recovery_account = recovery_account;
//                }
//            } else {
//                acc.recovery_account = "beowulf";
//            }
        }
        ...
        void account_create_evaluator::do_apply(const account_create_operation &o) {
            ...
            if (_db.has_hardfork(BEOWULF_HARDFORK_0_20__2651) || _db.is_producing()) {
                FC_TODO("Move to validate() after HF20");
                FC_ASSERT(o.fee <= asset(BEOWULF_MAX_ACCOUNT_CREATION_FEE, SBD_SYMBOL),
                          "Account creation fee cannot be too large");
            }

            if (_db.has_hardfork(BEOWULF_HARDFORK_0_20__1771)) {
                FC_ASSERT(o.fee == wso.median_props.account_creation_fee,
                          "Must pay the exact account creation fee. paid: ${p} fee: ${f}",
                          ("p", o.fee)
                                  ("f", wso.median_props.account_creation_fee));
            } else if (!_db.has_hardfork(BEOWULF_HARDFORK_0_20__1761) && _db.has_hardfork(BEOWULF_HARDFORK_0_19__987)) {
                FC_ASSERT(o.fee >=
                          asset(wso.median_props.account_creation_fee.amount * BEOWULF_CREATE_ACCOUNT_WITH_BEOWULF_MODIFIER,
                                SBD_SYMBOL), "Insufficient Fee: ${f} required, ${p} provided.",
                          ("f", wso.median_props.account_creation_fee *
                                asset(BEOWULF_CREATE_ACCOUNT_WITH_BEOWULF_MODIFIER, SBD_SYMBOL))
                                  ("p", o.fee));
            } else if (_db.has_hardfork(BEOWULF_HARDFORK_0_1)) {
                FC_ASSERT(o.fee >= wso.median_props.account_creation_fee,
            ...
            if (_db.is_producing() || _db.has_hardfork(BEOWULF_HARDFORK_0_20)) {
                validate_auth_size(o.owner);
                validate_auth_size(o.active);
//                validate_auth_size(o.posting);
            }

            if (_db.has_hardfork(BEOWULF_HARDFORK_0_15__465)) {
...
//            if (_db.has_hardfork(BEOWULF_HARDFORK_0_20__1762)) {
//                _db.adjust_balance(_db.get<account_object, by_name>(BEOWULF_NULL_ACCOUNT), o.fee);
//            }

            const auto &new_account = _db.create<account_object>([&](account_object &acc) {
                initialize_account_object(acc, o.new_account_name, props, false /*mined*/,
                                          _db.get_hardfork());
#ifndef IS_LOW_MEM
...
//                auth.posting = o.posting;
            ...
            if (!_db.has_hardfork(BEOWULF_HARDFORK_0_20__1762) && o.fee.amount > 0 && o.fee.symbol == BEOWULF_SYMBOL) {
                _db.create_vesting(new_account, o.fee);
            }
        }

//        void account_create_with_delegation_evaluator::do_apply(const account_create_with_delegation_operation &o) {
//            FC_ASSERT(!_db.has_hardfork(BEOWULF_HARDFORK_0_20__1760),
//                      "Account creation with delegation is deprecated as of Hardfork 20");
//
//            if (_db.has_hardfork(BEOWULF_HARDFORK_0_20__2651) || _db.is_producing()) {
//                FC_TODO("Move to validate() after HF20");
//                FC_ASSERT(o.fee <= asset(BEOWULF_MAX_ACCOUNT_CREATION_FEE, SBD_SYMBOL),
...
//                    asset(wso.median_props.account_creation_fee.amount * BEOWULF_CREATE_ACCOUNT_WITH_BEOWULF_MODIFIER *
//                          BEOWULF_CREATE_ACCOUNT_DELEGATION_RATIO, SBD_SYMBOL) * props.get_vesting_share_price();
//
//            auto current_delegation = asset(o.fee.amount * BEOWULF_CREATE_ACCOUNT_DELEGATION_RATIO, SBD_SYMBOL) *
...
//            });
//
//            if (o.delegation.amount > 0 || !_db.has_hardfork(BEOWULF_HARDFORK_0_19__997)) {
...
//                    vdo.min_delegation_time = _db.head_block_time() + BEOWULF_CREATE_ACCOUNT_DELEGATION_TIME;
//                });
//            }
//
//            if (!_db.has_hardfork(BEOWULF_HARDFORK_0_20__1762) && o.fee.amount > 0 && o.fee.symbol == BEOWULF_SYMBOL) {
...
//
        void account_update_evaluator::do_apply(const account_update_operation &o) {
            if (_db.has_hardfork(BEOWULF_HARDFORK_0_1))
                FC_ASSERT(o.account != BEOWULF_TEMP_ACCOUNT, "Cannot update temp account.");
//            if ((_db.has_hardfork(BEOWULF_HARDFORK_0_15__465)) && o.posting)
//                o.posting->validate();
...
//                    validate_auth_size(*o.posting);
            ...
            if (o.owner) {
#ifndef IS_TEST_NET
                if (_db.has_hardfork(BEOWULF_HARDFORK_0_11))
                    FC_ASSERT(_db.head_block_time() - account_auth.last_owner_update > BEOWULF_OWNER_UPDATE_LIMIT,
            ...
            if (o.active && (_db.has_hardfork(BEOWULF_HARDFORK_0_15__465)))
                verify_authority_accounts_exist(_db, *o.active, o.account, authority::active);
//            if (o.posting && (_db.has_hardfork(BEOWULF_HARDFORK_0_15__465)))
...
//
//   if( _db.has_hardfork( BEOWULF_HARDFORK_0_19__876 ) )
//      FC_ASSERT( comment.cashout_time != fc::time_point_sec::maximum() );
//
//   if( _db.has_hardfork( BEOWULF_HARDFORK_0_19__977 ) )
...
//   if( _db.has_hardfork( BEOWULF_HARDFORK_0_6__80 ) && comment.parent_author != BEOWULF_ROOT_POST_PARENT )
//   {
...
//   FC_ASSERT( comment.percent_beowulf_dollars >= o.percent_beowulf_dollars, "A comment cannot accept a greater percent SBD." );
//
//   _db.modify( comment, [&]( comment_object& c ) {
//       c.max_accepted_payout   = o.max_accepted_payout;
//       c.percent_beowulf_dollars = o.percent_beowulf_dollars;
...
//{ try {
//   if( _db.has_hardfork( BEOWULF_HARDFORK_0_5__55 ) )
...
//      if( !_db.has_hardfork( BEOWULF_HARDFORK_0_17__767 ) )
//         FC_ASSERT( parent->depth < BEOWULF_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",BEOWULF_MAX_COMMENT_DEPTH_PRE_HF17) );
//      else
//         FC_ASSERT( parent->depth < BEOWULF_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",BEOWULF_MAX_COMMENT_DEPTH) );
//   }
//
...
//         if( _db.has_hardfork( BEOWULF_HARDFORK_0_12__177 ) && !_db.has_hardfork( BEOWULF_HARDFORK_0_17__869 ) )
...
//      if( _db.has_hardfork( BEOWULF_HARDFORK_0_20__2019 ) )
//      {
//         if( o.parent_author == BEOWULF_ROOT_POST_PARENT )
//             FC_ASSERT( ( now - auth.last_root_post ) > BEOWULF_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
//         else
//             FC_ASSERT( (now - auth.last_post) >= BEOWULF_MIN_REPLY_INTERVAL_HF20, "You may only comment once every 3 seconds.", ("now",now)("auth.last_post",auth.last_post) );
//      }
//      else if( _db.has_hardfork( BEOWULF_HARDFORK_0_12__176 ) )
//      {
//         if( o.parent_author == BEOWULF_ROOT_POST_PARENT )
//             FC_ASSERT( ( now - auth.last_root_post ) > BEOWULF_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
//         else
//             FC_ASSERT( (now - auth.last_post) > BEOWULF_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
//      }
//      else if( _db.has_hardfork( BEOWULF_HARDFORK_0_6__113 ) )
//      {
//         if( o.parent_author == BEOWULF_ROOT_POST_PARENT )
//             FC_ASSERT( (now - auth.last_post) > BEOWULF_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("auth.last_post",auth.last_post) );
//         else
//             FC_ASSERT( (now - auth.last_post) > BEOWULF_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
//      }
//      else
...
//      }
//
//      uint16_t reward_weight = BEOWULF_100_PERCENT;
//      uint64_t post_bandwidth = auth.post_bandwidth;
//
//      if( _db.has_hardfork( BEOWULF_HARDFORK_0_12__176 ) && !_db.has_hardfork( BEOWULF_HARDFORK_0_17__733 ) && o.parent_author == BEOWULF_ROOT_POST_PARENT )
//      {
//         uint64_t post_delta_time = std::min( _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(), BEOWULF_POST_AVERAGE_WINDOW );
//         uint32_t old_weight = uint32_t( ( post_bandwidth * ( BEOWULF_POST_AVERAGE_WINDOW - post_delta_time ) ) / BEOWULF_POST_AVERAGE_WINDOW );
//         post_bandwidth = ( old_weight + BEOWULF_100_PERCENT );
//         reward_weight = uint16_t( std::min( ( BEOWULF_POST_WEIGHT_CONSTANT * BEOWULF_100_PERCENT ) / ( post_bandwidth * post_bandwidth ), uint64_t( BEOWULF_100_PERCENT ) ) );
//      }
//
...
//            com.cashout_time = _db.has_hardfork( BEOWULF_HARDFORK_0_12__177 ) ?
//               _db.head_block_time() + BEOWULF_CASHOUT_WINDOW_SECONDS_PRE_HF17 :
...
//         }
//
//         if( _db.has_hardfork( BEOWULF_HARDFORK_0_17__769 ) )
//         {
//            com.cashout_time = com.created + BEOWULF_CASHOUT_WINDOW_SECONDS;
//         }
//      });
//
//      id = new_comment.id;
...
//
//      if( !_db.has_hardfork( BEOWULF_HARDFORK_0_17__772 ) )
//      {
//         if( _db.has_hardfork( BEOWULF_HARDFORK_0_14__306 ) )
//            FC_ASSERT( _db.calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum(), "The comment is archived." );
//         else if( _db.has_hardfork( BEOWULF_HARDFORK_0_10 ) )
...
//
//      asset beowulf_spent = o.beowulf_amount;
//      asset sbd_spent = o.sbd_amount;
//      if( o.fee.symbol == BEOWULF_SYMBOL )
//         beowulf_spent += o.fee;
//      else
//         sbd_spent += o.fee;
//
//      FC_ASSERT( from_account.balance >= beowulf_spent, "Account cannot cover BEOWULF costs of escrow. Required: ${r} Available: ${a}", ("r",beowulf_spent)("a",from_account.balance) );
...
//
//      _db.adjust_balance( from_account, -beowulf_spent );
...
//         esc.beowulf_balance          = o.beowulf_amount;
...
//      if( reject_escrow )
//      {
//         _db.adjust_balance( o.from, escrow.beowulf_balance );
...
//
//      const auto& e = _db.get_escrow( o.from, o.escrow_id );
//      FC_ASSERT( e.beowulf_balance >= o.beowulf_amount, "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ("a", o.beowulf_amount)("b", e.beowulf_balance) );
...
//
//      _db.adjust_balance( o.receiver, o.beowulf_amount );
...
//      });
//
//      if( e.beowulf_balance.amount == 0 && e.sbd_balance.amount == 0 )
//      {
//         _db.remove( e );
//      }
//   }
...
//            if(_db.is_producing() )
//            {
//                FC_ASSERT( o.fee <= asset( BEOWULF_MAX_TRANSFER_FEE, o.fee.symbol ), "Transfer fee cannot be too large" );
//            }

            FC_ASSERT( o.fee >= asset (BEOWULF_MIN_TRANSFER_FEE, o.fee.symbol), "Insufficient fee. paid: ${p} fee: ${f}",
                       ("p", o.fee)
                               ("f", BEOWULF_MIN_TRANSFER_FEE) );
            ...
            if(o.fee.symbol == o.amount.symbol){
                ...
                //Reject transaction if not enough
                FC_ASSERT(_db.get_balance(o.from, o.fee.symbol) >= o.fee,
...
//                 else {
//                    //Account does not have enough SBD to pay fee, calculate fee by BEOWULF
//                    const auto& fhistory = _db.get_feed_history();
//                    FC_ASSERT( !fhistory.current_median_history.is_null(), "Cannot convert SBD because there is no price feed." );
//
//                    asset feeByBeowulf = util::to_beowulf(fhistory.current_median_history, o.fee);
//                    if (is_asset_type(o.amount, BEOWULF_SYMBOL)) {
//                        //If amount is also BEOWULF
//                        asset totalAmount = feeByBeowulf + o.amount;
//                        FC_ASSERT( _db.get_balance( o.from, o.amount.symbol ) >= totalAmount, "Account does not have sufficient funds for transfer." );
//                        _db.adjust_balance( o.from, -totalAmount );
//                    }else{
//                        //Amount is not BEOWULF
//                        FC_ASSERT(_db.get_balance(o.from, o.amount.symbol) >= o.amount,
//                                  "Account does not have sufficient funds for transfer.");
//                        FC_ASSERT(_db.get_balance(o.from, feeByBeowulf.symbol) >= feeByBeowulf, "Insufficient balance to pay fee.",
//                                  ("from.balance", _db.get_balance(o.from, feeByBeowulf.symbol))("required", feeByBeowulf));
//                        _db.adjust_balance(o.from, -o.amount);
//                        _db.adjust_balance(o.from, -feeByBeowulf);
//                    }
//                }
            }
        ...
        void withdraw_vesting_evaluator::do_apply(const withdraw_vesting_operation &o) {
            ...
            FC_ASSERT(account.vesting_shares >= asset(0, VESTS_SYMBOL),
                      "Account does not have sufficient Beowulf Power for withdraw.");
...
//                      "Account does not have sufficient Beowulf Power for withdraw.");
            ...
            if (!_db.has_hardfork(BEOWULF_HARDFORK_0_20__1860) && !account.mined &&
                _db.has_hardfork(BEOWULF_HARDFORK_0_1)) {
                const auto &props = _db.get_dynamic_global_properties();
                const witness_schedule_object &wso = _db.get_witness_schedule_object();
                ...
                FC_ASSERT(account.vesting_shares > min_vests ||
                          (_db.has_hardfork(BEOWULF_HARDFORK_0_16__562) && o.vesting_shares.amount == 0),
                          "Account registered by another account requires 10x account creation fee worth of Beowulf Power before it can be powered down.");
            }

            if (o.vesting_shares.amount == 0) {
                if (_db.has_hardfork(BEOWULF_HARDFORK_0_5__57))
                ...
                int vesting_withdraw_intervals = BEOWULF_VESTING_WITHDRAW_INTERVALS_PRE_HF_16;
                if (_db.has_hardfork(BEOWULF_HARDFORK_0_16__551))
                    vesting_withdraw_intervals = BEOWULF_VESTING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
                ...
                _db.modify(account, [&](account_object &a) {
                    ...
                    a.next_vesting_withdrawal =
                            _db.head_block_time() + fc::seconds(BEOWULF_VESTING_WITHDRAW_INTERVAL_SECONDS);
        ...
        void set_withdraw_vesting_route_evaluator::do_apply(const set_withdraw_vesting_route_operation &o) {
            try {
                ...
                if (itr == wd_idx.end()) {
                    ...
                    FC_ASSERT(from_account.withdraw_routes < BEOWULF_MAX_WITHDRAW_ROUTES,
                ...
                FC_ASSERT(total_percent <= BEOWULF_100_PERCENT,
...
//                proxy_chain.reserve(BEOWULF_MAX_PROXY_RECURSION_DEPTH + 1);
//
...
//                    cprox = &next_proxy;
//                    FC_ASSERT(proxy_chain.size() <= BEOWULF_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long.");
//                }
//
...
//                FC_ASSERT(voter.can_vote, "Account has declined its voting rights.");
            ...
            if (itr == by_account_witness_idx.end()) {
                ...
                if (_db.has_hardfork(BEOWULF_HARDFORK_0_2)) {
                    FC_ASSERT(voter.witnesses_voted_for < BEOWULF_MAX_ACCOUNT_WITNESS_VOTES,
                    ...
                    if (_db.has_hardfork(BEOWULF_HARDFORK_0_3)) {
...
//
//   if( _db.has_hardfork( BEOWULF_HARDFORK_0_12__177 ) && _db.calculate_discussion_payout_time( comment ) == fc::time_point_sec::maximum() )
//   {
//#ifndef CLEAR_VOTES
...
//
//   if( _db.has_hardfork( BEOWULF_HARDFORK_0_11 ) )
//      FC_ASSERT( elapsed_seconds >= BEOWULF_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );
//
//   int64_t regenerated_power = (BEOWULF_100_PERCENT * elapsed_seconds) / BEOWULF_VOTING_MANA_REGENERATION_SECONDS;
//   int64_t current_power     = std::min( int64_t(voter.voting_manabar.current_mana) + regenerated_power, int64_t(BEOWULF_100_PERCENT) );
...
//   int64_t  used_power  = ((current_power * abs_weight) / BEOWULF_100_PERCENT) * (60*60*24);
//
//   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();
//
//   // The second multiplication is rounded up as of HF 259
//   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * BEOWULF_VOTING_MANA_REGENERATION_SECONDS;
//   FC_ASSERT( max_vote_denom > 0 );
//
//   if( !_db.has_hardfork( BEOWULF_HARDFORK_0_14__259 ) )
//   {
...
//
//   int64_t abs_rshares    = ((uint128_t( _db.get_effective_vesting_shares( voter, VESTS_SYMBOL ).amount.value ) * used_power) / (BEOWULF_100_PERCENT)).to_uint64();
//   if( !_db.has_hardfork( BEOWULF_HARDFORK_0_14__259 ) && abs_rshares == 0 ) abs_rshares = 1;
//
//   if( _db.has_hardfork( BEOWULF_HARDFORK_0_20__1764 ) )
//   {
//      abs_rshares -= BEOWULF_VOTE_DUST_THRESHOLD;
//      abs_rshares = std::max( int64_t(0), abs_rshares );
//   }
//   else if( _db.has_hardfork( BEOWULF_HARDFORK_0_14__259 ) )
//   {
//      FC_ASSERT( abs_rshares > BEOWULF_VOTE_DUST_THRESHOLD || o.weight == 0, "Voting weight is too small, please accumulate more voting power or beowulf power." );
//   }
//   else if( _db.has_hardfork( BEOWULF_HARDFORK_0_13__248 ) )
//   {
//      FC_ASSERT( abs_rshares > BEOWULF_VOTE_DUST_THRESHOLD || abs_rshares == 1, "Voting weight is too small, please accumulate more voting power or beowulf power." );
//   }
//
//
//
...
//   {
//      if( _db.has_hardfork( BEOWULF_HARDFORK_0_12__177 ) )
...
//      if( rshares > 0 )
//      {
//         if( _db.has_hardfork( BEOWULF_HARDFORK_0_17__900 ) )
//            FC_ASSERT( _db.head_block_time() < comment.cashout_time - BEOWULF_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
//         else if( _db.has_hardfork( BEOWULF_HARDFORK_0_7 ) )
//            FC_ASSERT( _db.head_block_time() < _db.calculate_discussion_payout_time( comment ) - BEOWULF_UPVOTE_LOCKOUT_HF7, "Cannot increase payout within last minute before payout." );
//      }
//
...
//
//         if( _db.has_hardfork( BEOWULF_HARDFORK_0_12__177 ) && !_db.has_hardfork( BEOWULF_HARDFORK_0_13__257)  )
//            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + BEOWULF_CASHOUT_WINDOW_SECONDS_PRE_HF17;
//         else
//            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + BEOWULF_CASHOUT_WINDOW_SECONDS_PRE_HF12;
//
...
//         else
//            c.net_votes--;
//         if( !_db.has_hardfork( BEOWULF_HARDFORK_0_6__114 ) && c.net_rshares == -c.abs_rshares) FC_ASSERT( c.net_votes < 0, "Comment has negative net votes?" );
//      });
//
...
//         if( !_db.has_hardfork( BEOWULF_HARDFORK_0_17__769 ) )
//         {
//            if( _db.has_hardfork( BEOWULF_HARDFORK_0_12__177 ) && c.last_payout > fc::time_point_sec::min() )
...
//               c.max_cashout_time = _db.head_block_time() + fc::seconds( BEOWULF_MAX_CASHOUT_WINDOW_SECONDS );
//         }
//      });
//
...
//
//         if( curation_reward_eligible && _db.has_hardfork( BEOWULF_HARDFORK_0_17__774 ) )
//            curation_reward_eligible = _db.get_curation_rewards_percent( comment ) > 0;
//
//         if( curation_reward_eligible )
//         {
//            if( comment.created < fc::time_point_sec(BEOWULF_HARDFORK_0_6_REVERSE_AUCTION_TIME) ) {
...
//                  auto curve = !_db.has_hardfork( BEOWULF_HARDFORK_0_19__1052 ) && comment.created > BEOWULF_HF_19_SQRT_PRE_CALC
...
//
//            if( _db.head_block_time() > fc::time_point_sec(BEOWULF_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
//            {
...
//   else
//   {
//      FC_ASSERT( itr->num_changes < BEOWULF_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );
//
//      if( _db.has_hardfork( BEOWULF_HARDFORK_0_6__112 ) )
...
//            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + BEOWULF_CASHOUT_WINDOW_SECONDS_PRE_HF12;
//
//         if( _db.has_hardfork( BEOWULF_HARDFORK_0_14__259 ) && abs_rshares == 0 )
...
//
//   auto now = _db.head_block_time();
//   FC_ASSERT( ( now - voter.last_vote_time ).to_seconds() >= BEOWULF_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );
//
//   _db.modify( voter, [&]( account_object& a )
//   {
...
//   uint128_t used_mana = ( uint128_t( voter.voting_manabar.current_mana ) * abs_weight * 60 * 60 * 24 ) / BEOWULF_100_PERCENT;
//
//   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();
//
//   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * BEOWULF_VOTING_MANA_REGENERATION_SECONDS;
...
//
//   if( cashout_delta < BEOWULF_UPVOTE_LOCKOUT_SECONDS )
//   {
//      abs_rshares = (int64_t) ( ( uint128_t( abs_rshares ) * cashout_delta ) / BEOWULF_UPVOTE_LOCKOUT_SECONDS ).to_uint64();
//   }
//
//   if( itr == comment_vote_idx.end() )
//   {
...
//{ try {
//   if( _db.has_hardfork( BEOWULF_HARDFORK_0_20__2539 ) )
//   {
...
//
//            if (_db.is_producing() || _db.has_hardfork(BEOWULF_HARDFORK_0_20)) {
//                FC_ASSERT(o.required_auths.size() <= BEOWULF_MAX_AUTHORITY_MEMBERSHIP,
//                          "Too many auths specified. Max: 10, Current: ${n}", ("n", o.required_auths.size()));
//            }
//        }
//
...
//                FC_ASSERT(num_auths <= BEOWULF_MAX_AUTHORITY_MEMBERSHIP,
//                          "Too many auths specified. Max: 10, Current: ${n}", ("n", num_auths));
//            }
//
...
//            }
//            FC_ASSERT(d.has_hardfork(BEOWULF_HARDFORK_0_14__317));
//
//            if (_db.is_producing() || _db.has_hardfork(BEOWULF_HARDFORK_0_20)) {
//                size_t num_auths = o.required_owner_auths.size() + o.required_active_auths.size() +
//                                   o.required_posting_auths.size();
...
//
//            if (db.has_hardfork(BEOWULF_HARDFORK_0_5__59)) {
...
//                                              account_name_type(), db.get_hardfork());
...
//            if (db.has_hardfork(BEOWULF_HARDFORK_0_13__256))
...
//            if (db.head_block_num() < BEOWULF_START_MINER_VOTING_BLOCK)
//                pow_reward.amount *= BEOWULF_MAX_WITNESSES;
...
//            FC_ASSERT(!db().has_hardfork(BEOWULF_HARDFORK_0_13__256), "pow is deprecated. Use pow2 instead");
//            pow_apply(db(), o);
//        }
//
//
...
//            database &db = this->db();
//            FC_ASSERT(!db.has_hardfork(BEOWULF_HARDFORK_0_17__770), "mining is now disabled");
//
//            const auto &dgp = db.get_dynamic_global_properties();
//            uint32_t target_pow = db.get_pow_summary_target();
//            account_name_type worker_account;
//
//            if (db.has_hardfork(BEOWULF_HARDFORK_0_16__551)) {
...
//            }
//
//            FC_ASSERT(o.props.maximum_block_size >= BEOWULF_MIN_BLOCK_SIZE_LIMIT * 2,
...
//                                              account_name_type(), _db.get_hardfork());
//                    // ^ empty recovery account parameter means highest voted witness at time of recovery
//                });
//
//                db.create<account_authority_object>([&](account_authority_object &auth) {
...
//        }
        ...
        void feed_publish_evaluator::do_apply(const feed_publish_operation &o) {
            if (_db.has_hardfork(BEOWULF_HARDFORK_0_20__409))
                FC_ASSERT(is_asset_type(o.exchange_rate.base, SBD_SYMBOL) &&
                          is_asset_type(o.exchange_rate.quote, BEOWULF_SYMBOL),
                          "Price feed must be a SBD/BEOWULF price");
        ...
        void convert_evaluator::do_apply(const convert_operation &o) {
            ...
            auto beowulf_conversion_delay = BEOWULF_CONVERSION_DELAY_PRE_HF_16;
            if (_db.has_hardfork(BEOWULF_HARDFORK_0_16__551))
                beowulf_conversion_delay = BEOWULF_CONVERSION_DELAY;

            _db.create<convert_request_object>([&](convert_request_object &obj) {
                obj.owner = o.owner;
                obj.requestid = o.requestid;
                obj.amount = o.amount;
                obj.conversion_date = _db.head_block_time() + beowulf_conversion_delay;
            });
...
//
//   if( _db.has_hardfork( BEOWULF_HARDFORK_0_20__1449) )
//   {
//      FC_ASSERT( o.expiration <= _db.head_block_time() + BEOWULF_MAX_LIMIT_ORDER_EXPIRATION, "Limit Order Expiration must not be more than 28 days in the future" );
//   }
//
...
//          obj.expiration = std::min( o.expiration, fc::time_point_sec( BEOWULF_HARDFORK_0_20_TIME + BEOWULF_MAX_LIMIT_ORDER_EXPIRATION + rand_offset ) );
//       }
//   });
//
...
//       else
//       {
//          obj.expiration = std::min( o.expiration, fc::time_point_sec( BEOWULF_HARDFORK_0_20_TIME + BEOWULF_MAX_LIMIT_ORDER_EXPIRATION ) );
//       }
//   });
//
//   bool filled = _db.apply_order( order );
...
//}
        ...
        void report_over_production_evaluator::do_apply(const report_over_production_operation &o) {
            FC_ASSERT(!_db.has_hardfork(BEOWULF_HARDFORK_0_4), "report_over_production_operation is disabled.");
        }

//        void claim_account_evaluator::do_apply(const claim_account_operation &o) {
//            FC_ASSERT(_db.has_hardfork(BEOWULF_HARDFORK_0_20__1771),
//                      "claim_account_operation is not enabled until hardfork 20.");
//
//            const auto &creator = _db.get_account(o.creator);
...
//
//                    FC_ASSERT(current_witness.available_witness_account_subsidies >= BEOWULF_ACCOUNT_SUBSIDY_PRECISION,
//                              "Witness ${w} does not have enough subsidized accounts to claim",
//                              ("w", current_witness.owner));
//
//                    _db.modify(current_witness, [&](witness_object &w) {
//                        w.available_witness_account_subsidies -= BEOWULF_ACCOUNT_SUBSIDY_PRECISION;
//                    });
//                }
//
//                FC_ASSERT(gpo.available_account_subsidies >= BEOWULF_ACCOUNT_SUBSIDY_PRECISION,
//                          "There are not enough subsidized accounts to claim");
//
//                _db.modify(gpo, [&](dynamic_global_property_object &gpo) {
//                    gpo.available_account_subsidies -= BEOWULF_ACCOUNT_SUBSIDY_PRECISION;
//                });
//            } else {
//                FC_ASSERT(o.fee == wso.median_props.account_creation_fee,
...
//            }
//
//            //_db.adjust_balance(_db.get_account(BEOWULF_NULL_ACCOUNT), o.fee);
//
...
//            FC_ASSERT(_db.has_hardfork(BEOWULF_HARDFORK_0_20__1771),
//                      "create_claimed_account_operation is not enabled until hardfork 20.");
//
//            const auto &creator = _db.get_account(o.creator);
//            const auto &props = _db.get_dynamic_global_properties();
//
//            FC_ASSERT(creator.pending_claimed_accounts > 0, "${creator} has no claimed accounts to create",
//                      ("creator", o.creator));
//
//            verify_authority_accounts_exist(_db, o.owner, o.new_account_name, authority::owner);
//            verify_authority_accounts_exist(_db, o.active, o.new_account_name, authority::active);
...
//                if (o.recovery_account == BEOWULF_TEMP_ACCOUNT)
...
//                    req.expires = _db.head_block_time() + BEOWULF_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
//                });
...
//
//            if (_db.has_hardfork(BEOWULF_HARDFORK_0_12))
//                FC_ASSERT(_db.head_block_time() - account.last_account_recovery > BEOWULF_OWNER_UPDATE_LIMIT,
//                          "Owner authority can only be updated once an hour.");
//
//            const auto &recovery_request_idx = _db.get_index<account_recovery_request_index>().indices().get<by_account>();
//            auto request = recovery_request_idx.find(o.account_to_recover);
...
//                    req.effective_on = _db.head_block_time() + BEOWULF_OWNER_AUTH_RECOVERY_PERIOD;
//                });
...
//
//            FC_ASSERT(from.savings_withdraw_requests < BEOWULF_SAVINGS_WITHDRAW_REQUEST_LIMIT,
...
//                s.complete = _db.head_block_time() + BEOWULF_SAVINGS_WITHDRAW_TIME;
//            });
//
...
//            FC_ASSERT(_db.has_hardfork(BEOWULF_HARDFORK_0_14__324));
//
//            const auto &account = _db.get_account(o.account);
...
//                    req.effective_date = _db.head_block_time() + BEOWULF_OWNER_AUTH_RECOVERY_PERIOD;
//                });
//            } else {
...
//            const auto &acnt = _db.get_account(op.account);
//
//            FC_ASSERT(op.reward_beowulf <= acnt.reward_beowulf_balance,
//                      "Cannot claim that much BEOWULF. Claim: ${c} Actual: ${a}",
//                      ("c", op.reward_beowulf)("a", acnt.reward_beowulf_balance));
...
//
//            asset reward_vesting_beowulf_to_move = asset(0, BEOWULF_SYMBOL);
//            if (op.reward_vests == acnt.reward_vesting_balance)
//                reward_vesting_beowulf_to_move = acnt.reward_vesting_beowulf;
//            else
//                reward_vesting_beowulf_to_move = asset(
//                        ((uint128_t(op.reward_vests.amount.value) * uint128_t(acnt.reward_vesting_beowulf.amount.value))
//                         / uint128_t(acnt.reward_vesting_balance.amount.value)).to_uint64(), BEOWULF_SYMBOL);
//
//            _db.adjust_reward_balance(acnt, -op.reward_beowulf);
//            _db.adjust_reward_balance(acnt, -op.reward_sbd);
//            _db.adjust_balance(acnt, op.reward_beowulf);
//            _db.adjust_balance(acnt, op.reward_sbd);
//
//            _db.modify(acnt, [&](account_object &a) {
//                if (_db.has_hardfork(BEOWULF_HARDFORK_0_20__2539)) {
//                    util::manabar_params params(util::get_effective_vesting_shares(a),
//                                                BEOWULF_VOTING_MANA_REGENERATION_SECONDS);
...
//                a.reward_vesting_beowulf -= reward_vesting_beowulf_to_move;
//            });
//
//            _db.modify(_db.get_dynamic_global_properties(), [&](dynamic_global_property_object &gpo) {
//                gpo.total_vesting_shares += op.reward_vests;
//                gpo.total_vesting_fund_beowulf += reward_vesting_beowulf_to_move;
//
//                gpo.pending_rewarded_vesting_shares -= op.reward_vests;
//                gpo.pending_rewarded_vesting_beowulf -= reward_vesting_beowulf_to_move;
//            });
//
...
//               reward_vesting_beowulf_to_move = a->reward_vesting_beowulf;
//            else
//               reward_vesting_beowulf_to_move = asset( ( ( uint128_t( token.amount.value ) * uint128_t( a->reward_vesting_beowulf.amount.value ) )
//                  / uint128_t( a->reward_vesting_balance.amount.value ) ).to_uint64(), BEOWULF_SYMBOL );
//
...
//         }
//         else if( token.symbol == BEOWULF_SYMBOL || token.symbol == SBD_SYMBOL )
//         {
//            FC_ASSERT( is_asset_type( token, BEOWULF_SYMBOL ) == false || token <= a->reward_beowulf_balance,
//                       "Cannot claim that much BEOWULF. Claim: ${c} Actual: ${a}", ("c", token)("a", a->reward_beowulf_balance) );
...
//                    util::manabar_params params(max_mana, BEOWULF_VOTING_MANA_REGENERATION_SECONDS);
//                    a.voting_manabar.regenerate_mana(params, _db.head_block_time());
//                });
//
...
//                available_shares.amount = std::min(available_shares.amount,
//                                                   max_mana - delegator.received_vesting_shares.amount);
//
...
//
//                    available_shares +=
//                            weekly_withdraw - asset(delegator.to_withdraw - delegator.withdrawn, VESTS_SYMBOL);
//                }
//            } else {
//                available_shares = delegator.vesting_shares - delegator.delegated_vesting_shares -
...
//            //convert account_creation_fee to beowulf, then calculate
//            asset account_creation_fee_in_beowulf = _db.to_beowulf(wso.median_props.account_creation_fee);
//            auto min_delegation = _db.has_hardfork(BEOWULF_HARDFORK_0_20__1761) ?
//                                  asset(account_creation_fee_in_beowulf.amount / 3, BEOWULF_SYMBOL) *
//                                  gpo.get_vesting_share_price() :
//                                  asset(account_creation_fee_in_beowulf.amount * 10, BEOWULF_SYMBOL) *
//                                  gpo.get_vesting_share_price();
//            auto min_update = _db.has_hardfork(BEOWULF_HARDFORK_0_20__1761) ?
//                              asset(account_creation_fee_in_beowulf.amount / 30, BEOWULF_SYMBOL) *
//                              gpo.get_vesting_share_price() :
//                              account_creation_fee_in_beowulf * gpo.get_vesting_share_price();
//
//            // If delegation doesn't exist, create it
//            if (delegation == nullptr) {
//                FC_ASSERT(available_shares >= op.vesting_shares,
//                          "Account ${acc} does not have enough mana to delegate. required: ${r} available: ${a}",
...
//
//                FC_ASSERT(delta >= min_update, "Beowulf Power increase is not enough of a difference. min_update: ${min}",
//                          ("min", min_update));
//                FC_ASSERT(available_shares >= delta,
//                          "Account ${acc} does not have enough mana to delegate. required: ${r} available: ${a}",
...
//                              "Beowulf Power decrease is not enough of a difference. min_update: ${min}",
//                              ("min", min_update));
...
//                    obj.expiration = std::max(_db.head_block_time() + gpo.delegation_return_period,
