...
namespace beowulf { namespace wallet {

using namespace std;

using namespace beowulf::utilities;
...
{
   public:
      wallet_api( const wallet_data& initial_data, const beowulf::protocol::chain_id_type& _beowulf_chain_id, fc::api< remote_node_api > rapi );
      virtual ~wallet_api();
...
//         public_key_type memo,
       ...
       *  'info' wallet command.
       *
       *  These accounts are created with combination of BEOWULF and delegated SP
       *
       *  @param creator The account creating the new account
       *  @param beowulf_fee The amount of the fee to be paid with BEOWULF
...
//         bool broadcast = false);
      ...
      /**
       * Transfer funds from one account to another. BEOWULF and SBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 BEOWULF"
      ...
      /**
       * Transfer funds from one account to another using escrow. BEOWULF and SBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       ...
       * @param beowulf_amount The amount of BEOWULF to transfer
...
//      );
       ...
       * @param beowulf_amount The amount of BEOWULF that will be released
       * @param broadcast true if you wish to broadcast the transaction
       */
...
//      );
      ...
      /**
       * Transfer BEOWULF into a vesting fund represented by vesting shares (VESTS). VESTS are required to vesting
       * for a minimum of one coin year and can be withdrawn once a week over a two year withdraw period.
       * VESTS are protected against dilution up until 90% of BEOWULF is vesting.
       *
       * @param from The account the BEOWULF is coming from
       * @param to The account getting the VESTS
       * @param amount The amount of BEOWULF to vest i.e. "100.00 BEOWULF"
       * @param broadcast true if you wish to broadcast the transaction
       */
...
//         bool broadcast = false );
       ...
       *    withdrawn and deposited back as BEOWULF. i.e. "10.000000 VESTS"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *
       * @param from The account the VESTS are withdrawn from.
       * @param to   The account receiving either VESTS or BEOWULF.
      ...
      /**
       *  This method will convert SBD to BEOWULF at the current_median_history price one
      ...
      /**
       * A witness can public a price feed for the BEOWULF:SBD market. The median price feed is used
       * to process conversion requests from SBD to BEOWULF.
       *
       ...
       *                       operations defined in `beowulf/chain/operations.hpp`
      ...
      /**
       * Gets the current order book for BEOWULF:SBD
       *
...
//      vector< condenser_api::api_limit_order_object > get_open_orders( string accountname );
       ...
       *  @param amount_to_sell The amount of either SBD or BEOWULF you wish to sell
...
//         bool broadcast );
      ...
      /**
       * Vote on a comment to be paid BEOWULF
       *
...
FC_REFLECT( beowulf::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( beowulf::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( beowulf::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( beowulf::wallet::authority_type, (owner)(active) )

FC_API( beowulf::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
...
FC_REFLECT( beowulf::wallet::memo_data, (from)(to)(nonce)(check)(encrypted) )
