...
{
   static_assert( BEOWULF_ASSET_SYMBOL_NAI_STRING_LENGTH >= 12, "This code will overflow a short buffer" );
   uint32_t x = to_nai();
   buf[11] = '\0';
   buf[10] = ((x%10)+'0');  x /= 10;
   buf[ 9] = ((x%10)+'0');  x /= 10;
...
asset_symbol_type asset_symbol_type::from_nai_string( const char* p, uint8_t decimal_places )
{
   try
   {
      ...
      FC_ASSERT( std::strlen( p ) == BEOWULF_ASSET_SYMBOL_NAI_STRING_LENGTH - 1, "Incorrect NAI string length" );
...
uint32_t asset_symbol_type::asset_num_from_nai( uint32_t nai, uint8_t decimal_places )
{
   ...
   {
      case BEOWULF_NAI_BEOWULF:
         FC_ASSERT( decimal_places == BEOWULF_PRECISION_BEOWULF );
         return BEOWULF_ASSET_NUM_BEOWULF;
      case BEOWULF_NAI_SBD:
         FC_ASSERT( decimal_places == BEOWULF_PRECISION_SBD );
         return BEOWULF_ASSET_NUM_SBD;
      case BEOWULF_NAI_VESTS:
         FC_ASSERT( decimal_places == BEOWULF_PRECISION_VESTS );
         return BEOWULF_ASSET_NUM_VESTS;
      default:
         FC_ASSERT( decimal_places <= BEOWULF_ASSET_MAX_DECIMALS, "Invalid decimal_places" );
         return (nai_data_digits << BEOWULF_NAI_SHIFT) | SMT_ASSET_NUM_CONTROL_MASK | decimal_places;
   }
}
...
uint32_t asset_symbol_type::to_nai()const
{
   ...
   switch( asset_num )
   {
      case BEOWULF_ASSET_NUM_BEOWULF:
         nai_data_digits = BEOWULF_NAI_BEOWULF;
         break;
      case BEOWULF_ASSET_NUM_SBD:
         nai_data_digits = BEOWULF_NAI_SBD;
         break;
      case BEOWULF_ASSET_NUM_VESTS:
         nai_data_digits = BEOWULF_NAI_VESTS;
         break;
      default:
         FC_ASSERT( space() == smt_nai_space );
         nai_data_digits = (asset_num >> BEOWULF_NAI_SHIFT);
   }
...
asset_symbol_type asset_symbol_type::get_paired_symbol() const
{
   switch( space() )
   {
      case legacy_space:
      {
         ...
         {
            case BEOWULF_ASSET_NUM_BEOWULF:
               return from_asset_num( BEOWULF_ASSET_NUM_VESTS );
            case BEOWULF_ASSET_NUM_SBD:
               return *this;
            case BEOWULF_ASSET_NUM_VESTS:
               return from_asset_num( BEOWULF_ASSET_NUM_BEOWULF );
            default:
               FC_ASSERT( false, "Unknown asset symbol" );
         }
      }
...
void asset_symbol_type::validate()const
{
   switch( asset_num )
   {
      ...
      default:
      {
         uint32_t nai_data_digits = (asset_num >> BEOWULF_NAI_SHIFT);
...
void asset::validate()const
{
   ...
   FC_ASSERT( amount.value <= BEOWULF_MAX_SATOSHIS );
}
...
DEFINE_PRICE_COMPARISON_OPERATOR( >= )
      ...
      price price::max( asset_symbol_type base, asset_symbol_type quote ) { return asset( share_type(BEOWULF_MAX_SATOSHIS), base ) / asset( share_type(1), quote); }
      price price::min( asset_symbol_type base, asset_symbol_type quote ) { return asset( 1, base ) / asset( BEOWULF_MAX_SATOSHIS, quote); }
...
namespace fc {
   void to_variant( const beowulf::protocol::asset& var, fc::variant& vo )
   {
      try
      {
   ...
   void from_variant( const fc::variant& var, beowulf::protocol::asset& vo )
   {
      try
      {
         ...
         vo.symbol = beowulf::protocol::asset_symbol_type::from_nai_string( v_object[ ASSET_NAI_KEY ].as< std::string >().c_str(), v_object[ ASSET_PRECISION_KEY ].as< uint8_t >() );
      } FC_CAPTURE_AND_RETHROW()
   }
}
