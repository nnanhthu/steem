...
#define BEOWULF_ASSET_SYMBOL_PRECISION_BITS    4
#define BEOWULF_ASSET_CONTROL_BITS             1
#define BEOWULF_NAI_SHIFT                      ( BEOWULF_ASSET_SYMBOL_PRECISION_BITS + BEOWULF_ASSET_CONTROL_BITS )
#define SMT_MAX_NAI                          99999999
#define SMT_MIN_NAI                          1
#define SMT_MIN_NON_RESERVED_NAI             10000000
#define BEOWULF_ASSET_SYMBOL_NAI_LENGTH        10
#define BEOWULF_ASSET_SYMBOL_NAI_STRING_LENGTH ( BEOWULF_ASSET_SYMBOL_NAI_LENGTH + 2 )
#define SMT_MAX_NAI_POOL_COUNT               10
#define SMT_MAX_NAI_GENERATION_TRIES         100

#define BEOWULF_PRECISION_SBD   (5)
#define BEOWULF_PRECISION_BEOWULF (5)
#define BEOWULF_PRECISION_VESTS (6)
...
#define BEOWULF_NAI_SBD   (1)
#define BEOWULF_NAI_BEOWULF (2)
#define BEOWULF_NAI_VESTS (3)

#define BEOWULF_ASSET_NUM_SBD \
  (uint32_t(((SMT_MAX_NAI + BEOWULF_NAI_SBD)   << BEOWULF_NAI_SHIFT) | BEOWULF_PRECISION_SBD))
#define BEOWULF_ASSET_NUM_BEOWULF \
  (uint32_t(((SMT_MAX_NAI + BEOWULF_NAI_BEOWULF) << BEOWULF_NAI_SHIFT) | BEOWULF_PRECISION_BEOWULF))
#define BEOWULF_ASSET_NUM_VESTS \
  (uint32_t(((SMT_MAX_NAI + BEOWULF_NAI_VESTS) << BEOWULF_NAI_SHIFT) | BEOWULF_PRECISION_VESTS))

#ifdef IS_TEST_NET

#define VESTS_SYMBOL_U64  (uint64_t('V') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
#define BEOWULF_SYMBOL_U64  (uint64_t('T') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
...
#define BEOWULF_SYMBOL_U64  (uint64_t('B') | (uint64_t('W') << 8) | (uint64_t('F') << 16)) //BWF
...
#define BEOWULF_SYMBOL_SER  (uint64_t(5) | (BEOWULF_SYMBOL_U64 << 8)) ///< BEOWULF|TESTS with 5 digits of precision
#define SBD_SYMBOL_SER    (uint64_t(5) |   (SBD_SYMBOL_U64 << 8)) ///< SBD|TBD with 5 digits of precision

#define BEOWULF_ASSET_MAX_DECIMALS 12
...
class asset_symbol_type
{
   public:
      ...
      // buf must have space for BEOWULF_ASSET_SYMBOL_MAX_LENGTH+1
      ...
      {
         char buf[ BEOWULF_ASSET_SYMBOL_NAI_STRING_LENGTH ];
         to_nai_string( buf );
...
FC_REFLECT(beowulf::protocol::asset_symbol_type, (asset_num))
...
inline void pack( Stream& s, const beowulf::protocol::asset_symbol_type& sym )
{
   switch( sym.space() )
   {
      case beowulf::protocol::asset_symbol_type::legacy_space:
      {
         uint64_t ser = 0;
         switch( sym.asset_num )
         {
            case BEOWULF_ASSET_NUM_BEOWULF:
               ser = BEOWULF_SYMBOL_SER;
               break;
      ...
      case beowulf::protocol::asset_symbol_type::smt_nai_space:
...
template< typename Stream >
inline void unpack( Stream& s, beowulf::protocol::asset_symbol_type& sym, uint32_t )
{
   uint64_t ser = 0;
   s.read( (char*) &ser, 4 );
   ...
   switch( ser )
   {
      case BEOWULF_SYMBOL_SER & 0xFFFFFFFF:
         s.read( ((char*) &ser)+4, 4 );
         FC_ASSERT( ser == BEOWULF_SYMBOL_SER, "invalid asset bits" );
         sym.asset_num = BEOWULF_ASSET_NUM_BEOWULF;
         break;
      case SBD_SYMBOL_SER & 0xFFFFFFFF:
         s.read( ((char*) &ser)+4, 4 );
         FC_ASSERT( ser == SBD_SYMBOL_SER, "invalid asset bits" );
         sym.asset_num = BEOWULF_ASSET_NUM_SBD;
         break;
      case VESTS_SYMBOL_SER & 0xFFFFFFFF:
         s.read( ((char*) &ser)+4, 4 );
         FC_ASSERT( ser == VESTS_SYMBOL_SER, "invalid asset bits" );
         sym.asset_num = BEOWULF_ASSET_NUM_VESTS;
         break;
      default:
...
inline void to_variant( const beowulf::protocol::asset_symbol_type& sym, fc::variant& var )
{
   try
   {
...
inline void from_variant( const fc::variant& var, beowulf::protocol::asset_symbol_type& sym )
{
   ...
   try
   {
      ...
      FC_ASSERT( decimals->value().as_uint64() <= BEOWULF_ASSET_MAX_DECIMALS,
         "Expected decimals to be less than or equal to ${num}", ("num", BEOWULF_ASSET_MAX_DECIMALS) );
