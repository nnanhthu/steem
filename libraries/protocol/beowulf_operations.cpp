...
namespace beowulf { namespace protocol {
   ...
   void validate_auth_size( const authority& a )
   {
      ...
      FC_ASSERT( size <= BEOWULF_MAX_AUTHORITY_MEMBERSHIP, "Authority membership exceeded. Max: 10 Current: ${n}", ("n", size) );
   }
   ...
   void account_create_operation::validate() const
   {
      ...
      FC_ASSERT( fee >= asset( 0, SBD_SYMBOL ), "Account creation fee cannot be negative" );
   }
...
//      FC_ASSERT( beneficiaries[0].weight <= BEOWULF_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
//      sum += beneficiaries[0].weight;
//      FC_ASSERT( sum <= BEOWULF_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow
//
//      for( size_t i = 1; i < beneficiaries.size(); i++ )
//      {
//         validate_account_name( beneficiaries[i].account );
//         FC_ASSERT( beneficiaries[i].weight <= BEOWULF_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
...
//   {
//      validate_account_name( author );
//      FC_ASSERT( percent_beowulf_dollars <= BEOWULF_100_PERCENT, "Percent cannot exceed 100%" );
...
//   {
//      validate_account_name( creator );
//      FC_ASSERT( is_asset_type( fee, BEOWULF_SYMBOL ), "Account creation fee must be BOWC" );
//      FC_ASSERT( fee >= asset( 0, BEOWULF_SYMBOL ), "Account creation fee cannot be negative" );
//      FC_ASSERT( fee <= asset( BEOWULF_MAX_ACCOUNT_CREATION_FEE, SBD_SYMBOL ), "Account creation fee cannot be too large" );
//
...
//      validate_account_name( author );
//      FC_ASSERT( abs(weight) <= BEOWULF_100_PERCENT, "Weight is not a BEOWULFIT percentage" );
//      validate_permlink( permlink );
//   }
   ...
   { try {
      ...
      FC_ASSERT( is_asset_type( fee, SBD_SYMBOL ), "Transaction fee must be W" );
      ...
      FC_ASSERT( memo.size() < BEOWULF_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   void transfer_to_vesting_operation::validate() const
   {
      ...
      FC_ASSERT( amount.symbol == BEOWULF_SYMBOL ||
                 ...
                 "Amount must be BWF or SMT liquid" );
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= BEOWULF_100_PERCENT, "Percent must be valid beowulf percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( url.size() <= BEOWULF_MAX_WITNESS_URL_LENGTH, "URL is too long" );

      FC_ASSERT( url.size() > 0, "URL size must be greater than 0" );
      FC_ASSERT( fc::is_utf8( url ), "URL is not valid UTF8" );
      FC_ASSERT( fee >= asset( 0, SBD_SYMBOL ), "Fee cannot be negative" );
   ...
   void witness_set_properties_operation::validate() const
   {
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_creation_fee.amount >= BEOWULF_MIN_ACCOUNT_CREATION_FEE, "account_creation_fee smaller than minimum account creation fee" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( maximum_block_size >= BEOWULF_MIN_BLOCK_SIZE_LIMIT, "maximum_block_size smaller than minimum max block size" );
      }
...
//         FC_ASSERT( sbd_interest_rate <= BEOWULF_100_PERCENT, "sbd_interest_rate must not exceed 100%" );
//      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( ( is_asset_type( sbd_exchange_rate.base, SBD_SYMBOL ) && is_asset_type( sbd_exchange_rate.quote, BEOWULF_SYMBOL ) ),
            "Price feed must be a BEOWULF/SBD price" );
...
//         FC_ASSERT( account_subsidy_budget >= BEOWULF_RD_MIN_BUDGET, "Budget must be at least ${n}", ("n", BEOWULF_RD_MIN_BUDGET) );
//         FC_ASSERT( account_subsidy_budget <= BEOWULF_RD_MAX_BUDGET, "Budget must be at most ${n}", ("n", BEOWULF_RD_MAX_BUDGET) );
//      }
...
//         FC_ASSERT( account_subsidy_decay >= BEOWULF_RD_MIN_DECAY, "Decay must be at least ${n}", ("n", BEOWULF_RD_MIN_DECAY) );
//         FC_ASSERT( account_subsidy_decay <= BEOWULF_RD_MAX_DECAY, "Decay must be at most ${n}", ("n", BEOWULF_RD_MAX_DECAY) );
//      }
   }
...
//      proof = fc::equihash::proof::hash( BEOWULF_EQUIHASH_N, BEOWULF_EQUIHASH_K, seed );
...
//      validate_account_name( input.worker_account );
//      auto seed = fc::sha256::hash( input );
//      FC_ASSERT( proof.n == BEOWULF_EQUIHASH_N, "proof of work 'n' value is incorrect" );
//      FC_ASSERT( proof.k == BEOWULF_EQUIHASH_K, "proof of work 'k' value is incorrect" );
...
//   }
   ...
   void feed_publish_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( exchange_rate.base, BEOWULF_SYMBOL ) && is_asset_type( exchange_rate.quote, SBD_SYMBOL ) )
         || ( is_asset_type( exchange_rate.base, SBD_SYMBOL ) && is_asset_type( exchange_rate.quote, BEOWULF_SYMBOL ) ),
         "Price feed must be a BEOWULF/SBD price" );
...
//      validate_account_name( owner );
//
//      FC_ASSERT(  ( is_asset_type( amount_to_sell, BEOWULF_SYMBOL ) && is_asset_type( min_to_receive, SBD_SYMBOL ) )
//               || ( is_asset_type( amount_to_sell, SBD_SYMBOL ) && is_asset_type( min_to_receive, BEOWULF_SYMBOL ) )
//               || (
//                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
//                     && is_asset_type( min_to_receive, BEOWULF_SYMBOL )
//                  )
//               || (
//                     is_asset_type( amount_to_sell, BEOWULF_SYMBOL )
//                     && min_to_receive.symbol.space() == asset_symbol_type::smt_nai_space
//                  ),
//               "Limit order must be for the BEOWULF:SBD or SMT:(BEOWULF/SBD) market" );
//
...
//      exchange_rate.validate();
//
//      FC_ASSERT(  ( is_asset_type( amount_to_sell, BEOWULF_SYMBOL ) && is_asset_type( exchange_rate.quote, SBD_SYMBOL ) )
//               || ( is_asset_type( amount_to_sell, SBD_SYMBOL ) && is_asset_type( exchange_rate.quote, BEOWULF_SYMBOL ) )
//               || (
//                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
//                     && is_asset_type( exchange_rate.quote, BEOWULF_SYMBOL )
//                  )
//               || (
//                     is_asset_type( amount_to_sell, BEOWULF_SYMBOL )
...
//   }
   ...
   void convert_operation::validate()const
   {
      ...
      validate_account_name( owner );
      /// only allow conversion from SBD to BEOWULF, allowing the opposite can enable traders to abuse
...
//      FC_ASSERT( beowulf_amount.amount >= 0, "beowulf amount cannot be negative" );
//      FC_ASSERT( sbd_amount.amount > 0 || beowulf_amount.amount > 0, "escrow must transfer a non-zero amount" );
//      FC_ASSERT( from != agent && to != agent, "agent must be a third party" );
//      FC_ASSERT( (fee.symbol == BEOWULF_SYMBOL) || (fee.symbol == SBD_SYMBOL), "fee must be BEOWULF or SBD" );
//      FC_ASSERT( sbd_amount.symbol == SBD_SYMBOL, "sbd amount must contain SBD" );
//      FC_ASSERT( beowulf_amount.symbol == BEOWULF_SYMBOL, "beowulf amount must contain BEOWULF" );
...
//      FC_ASSERT( sbd_amount.amount >= 0, "sbd amount cannot be negative" );
//      FC_ASSERT( beowulf_amount.amount >= 0, "beowulf amount cannot be negative" );
//      FC_ASSERT( sbd_amount.amount > 0 || beowulf_amount.amount > 0, "escrow must release a non-zero amount" );
//      FC_ASSERT( sbd_amount.symbol == SBD_SYMBOL, "sbd amount must contain SBD" );
//      FC_ASSERT( beowulf_amount.symbol == BEOWULF_SYMBOL, "beowulf amount must contain BEOWULF" );
//   }
...
//      FC_ASSERT( amount.symbol == BEOWULF_SYMBOL || amount.symbol == SBD_SYMBOL );
//      FC_ASSERT( memo.size() < BEOWULF_MAX_MEMO_SIZE, "Memo is too large" );
//      FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
//   }
...
//   {
//      validate_account_name( account );
//      FC_ASSERT( is_asset_type( reward_beowulf, BEOWULF_SYMBOL ), "Reward Steem must be BEOWULF" );
...
//      FC_ASSERT( reward_beowulf.amount >= 0, "Cannot claim a negative amount" );
...
//      FC_ASSERT( reward_beowulf.amount > 0 || reward_sbd.amount > 0 || reward_vests.amount > 0, "Must claim something." );
//   }
...
//   }
   ...
   void convert_to_sbd_operation::validate()const
   {
      ...
      validate_account_name( owner );
      /// only convert from BEOWULF TO SBD, to convert from SBD to BEOWULF, using convert_operation.
      FC_ASSERT( is_asset_type( amount, BEOWULF_SYMBOL ), "Can only convert WBF to W" );
